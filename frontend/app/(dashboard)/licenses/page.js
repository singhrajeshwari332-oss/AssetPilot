...
export default function LicensesPage() {
  ...
  const handleRevokeSeat = async () => {
    ...
    try {
      await api.put(`/license-allocations/${revokeModal.allocation.id}/revoke`)
      toast.success("✓ License seat revoked successfully.")
      setRevokeModal({ open: false, allocation: null, licenseName: "", loading: false })
      fetchLicenses()
    } catch (err) {
      const msg = err.response?.data?.message || "✕ Failed to revoke license seat."
      toast.error(msg)
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 3 }).map((_, idx) => (
            <Card key={idx} className="h-72 animate-pulse bg-muted/40" />
          ))}
        </div>
      ) : licenses.length === 0 ? (
        <EmptyState
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {licenses.map((lic) => {
            const isFull = lic.allocatedSeats >= lic.seatQuota && lic.seatQuota > 0
            const percentageUsed = lic.seatQuota > 0 ? Math.round((lic.allocatedSeats / lic.seatQuota) * 100) : 0

            return (
              <Card key={lic.id} className="flex flex-col justify-between shadow-sm hover:border-primary/40 transition-colors">
                <div>
                    ...
                    <div className="space-y-2 border-t pt-3">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                          Active Allocations ({lic.activeAllocations?.length || 0})
                        </span>
                      </div>
                            ...
                            >
                              <div className="flex flex-col">
                                ...
                                <span className="text-[10px] text-muted-foreground">
                                  {alloc.employee?.department} &bull; {alloc.notes || "Assigned"}
                                </span>
                              </div>
                              <div className="flex items-center gap-2">
                ...
                <CardFooter className="p-5 pt-0 border-t bg-muted/10 rounded-b-lg mt-2">
                  ...
                  >
                    ...
                    <span>{isFull ? "Seat Quota Full" : "Allocate Seat"}</span>
                  </Button>
                </CardFooter>
              </Card>
            )
      ...
      <AllocateSeatDialog
        ...
        onOpenChange={(open) => setAllocateModal((prev) => ({ ...prev, open }))}
