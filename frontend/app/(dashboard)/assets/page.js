...
export default function AssetsPage() {
  ...
  const fetchAssets = useCallback(async () => {
    try {
      ...
      const params = {
        page: isEmployee ? 1 : page,
        limit: isEmployee ? 1000 : 10,
        search: search.trim(),
        category,
        status: isEmployee ? "ASSIGNED" : status,
      }
  ...
  (res.data?.data || []).map((asset) => ({
    ...
    status: asset.status,
    employeeId: asset.currentEmployee?.employeeId,
    employeeName: asset.currentEmployee?.name,
      ...
      /*
       * EMPLOYEE:
       * Only show assets currently assigned to the logged-in employee.
       *
       * This also removes an asset from My Assets immediately after
       * its status changes to RETURN_REQUESTED.
       */
      if (isEmployee) {
        fetchedAssets = fetchedAssets.filter((asset) => {
          const belongsToEmployee =
            asset?.currentEmployee?.id === user?.employeeId ||
            asset?.currentEmployee?.employeeId === user?.employeeId
        ...
        const filteredTotal = fetchedAssets.length
        const limit = 10
        ...
        const paginatedAssets = fetchedAssets.slice(
          startIndex,
          startIndex + limit
        )
  ...
  const handleDeleteAsset = async () => {
    if (!deleteModal.asset || !isSuperAdmin) return
    ...
    try {
      ...
      toast.success(
        `✓ Asset ${deleteModal.asset.assetTag} deleted successfully.`
      )
      ...
      if (
        drawerOpen &&
        selectedAssetId === deleteModal.asset.id
      ) {
        setDrawerOpen(false)
      }
  ...
  const isOwnAssignedAsset = (asset) => {
    if (!isEmployee || !asset?.currentEmployee) {
      return false
    }
  ...
  const canRequestReturn = (asset) => {
    return (
      isSuperAdmin ||
      (
        isEmployee &&
        asset?.status === "ASSIGNED" &&
        isOwnAssignedAsset(asset)
      )
    )
  }

  const canReportRepair = (asset) => {
    return (
      isEmployee &&
      asset?.status === "ASSIGNED" &&
      isOwnAssignedAsset(asset)
              ...
              <TableRow>
                ...
                >
                  <EmptyState
                    ...
                    description={
                      isEmployee
                        ? "You currently have no assigned company assets."
                        : "No inventory records matched your search parameters or filter criteria."
                    }
                    actionLabel={
                      isSuperAdmin
                        ? "Add New Asset"
